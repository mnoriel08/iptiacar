...
pistaBordeMin     = 60			# ancho minimo borde de pista
...
def avanzarAProxEsquina():
	...
	print("autoDistancia-alfaNum = " + autoDistancia)
	autoDistancia = float(autoDistancia)
	print("autoDistancia-float = " + autoDistancia)
	logging.info("Distancia hasta el borde negro de la pista: " + str(autoDistancia))
...
def direccionDelAuto():	# en que direccion se mueve el auto-robot?
	...
	logging.info("Distancia hasta el borde negro izquierdo: " + str(izqDistancia))
	...
	logging.info("Distancia hasta el borde negro derecho: " + str(derDistancia))
	...
	if izqDistancia < derDistancia:
		...
		logging.info("El auto-robot se mueve segun las manecillas de reloj...")
		ix.forward(0)	# detener el auto-robot
		for i in range(0,89):	# girar el auto-robot 90 grados
			...
			time.sleep(0.5)
		ix.forward(0)
		logging.info("listo para recorrer el siguiente tramo recto...")
		ix.forward(0)
		...
		logging.info("El auto-robot se mueve contra las manecillas del reloj...")
		ix.forward(0)	# detener el auto-robot
		for i in range(0,89):	# girar el auto-robot 90 grados
			ix.set_dir_servo_angle(-1)
			ix.forward(autoVelocidad)
